...
void Window::subWindowActivated(QMdiSubWindow * w)
{
  ...
  FActiveWidget=dynamic_cast<GLWindow*>(w->widget());
...
void Window :: bascule(TView type)
{
  ...
  {
    if (getActiveSubWindow()==(GLWindow*)vues.at(actu)->widget())
      find = true;
    else
      ++actu;
  }
  ...
  {
    if (getActiveSubWindow()->getViewType()!=type)
    {
...
GLWindow* Window::getActiveSubWindow() const
{
  assert(FActiveWidget!=nullptr);
  return FActiveWidget;
}

TViewId Window :: getCurrentViewId() const
{ return getActiveSubWindow()->getCliquedViewId(); }
...
void Window :: callbackKeyUp()
{
  ...
  switch (getActiveSubWindow()->getViewType())
  {
    case VIEW_XYZ :
    {
...
{
  if (getActiveSubWindow()->getViewType()==VIEW_XYZ)
  {
...
{
  GLWindow* active=getActiveSubWindow();
